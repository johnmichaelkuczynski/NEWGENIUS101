...
import { FolderOpen, Upload, Trash2, Download, FileText, X, Send } from "lucide-react";
      ...
      {open && (
        ...
        >
          <div className="bg-background border rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
            ...
            {/* Drop zone */}
            <div
              className={`mx-4 mt-4 border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors ${dragging ? "border-primary bg-primary/5" : "border-muted-foreground/30 hover:border-primary/50"}`}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              {!isLoading && docs.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-8">No documents yet. Upload your first file above.</p>
              )}
              {docs.map(doc => (
                <div
                  key={doc.id}
