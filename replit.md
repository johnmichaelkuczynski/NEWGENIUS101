...
- Login is REQUIRED (user order, July 5, 2026): all `/api/*` routes return 401 unless signed in (middleware in routes.ts right after `setupAuth`, exempting only `/api/auth/*`); frontend `AuthGate` in App.tsx blocks the whole UI behind a Google sign-in page. `/zhi/query` is outside `/api` and keeps its own key auth. Guest session IDs (`getSessionId`) still key chat data behind the wall — signing in does not change chat data.
